...
const Case: React.FC<CaseProps> = ({
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between gap-4 w-full">
      {/* Image */}
      <div className="flex items-center order-1 self-center rounded-xl overflow-hidden w-[186px] min-w-[186px] h-[186px]">
        {!loading && (
          <>
            {imageURL ? (
              <img
        ...
        {loading && (
          <Skeleton height={"186px"} width={"186px"} mt={6} radius="0.75rem" />
        )}
      </div>
      {/* Title & Meta Data */}
      <div className="order-2 sm:order-1 grow">
        {!loading && (
          <>
            {/* Title */}
